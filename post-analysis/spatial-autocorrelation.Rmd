...
```{r}
pr_tracts_demo <- geo_join(pr_tracts_demo, d_fema, by_sp="GEOID", by_df="censustractid", how="inner")
nrow(pr_tracts_demo)
```
...
```{r}
moran.test(pr_tracts_demo$below_poverty_rate, nblist, zero.policy = T, randomisation = FALSE)
```


### Check Moran's I for owner_occupied_rate
```{r}
moran.test(pr_tracts_demo$owner_occupied_rate, nblist, zero.policy = T, randomisation = FALSE)
```


### Check Moran's I for pwg_mph
```{r}
moran.test(pr_tracts_demo$pwg_mph, nblist, zero.policy = T, randomisation = FALSE)
```
