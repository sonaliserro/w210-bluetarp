...
d_fema <- d_puerto[ , .(householdcomposition = as.numeric(mean(householdcomposition)),
                        ...
                        waterlevel = as.numeric(mean(waterlevel, rm.na = TRUE)),
                        ...
                        damageamount = sum(damageamount)), keyby = c('state', 'county_fips', 'censustractid')]
...
d_fema[ , `:=` (specialneeds_cap = specialneeds/total_population,
                ...
                personalpropertyeligible_cap = personalpropertyeligible/total_population), keyby = c('state', 'county_fips', 'censustractid')]
d_fema
```
...
d_fema <- fread(file = '../data/open-fema/FEMA-Large-Tract-Demographics-WindSpeed-PR.csv')
```

```{r, fig.width=8, fig.height=3}
# Demographic variables
demo_cols <- c('below_poverty_rate', 'unemployed_labor_rate', 'built_1979_or_earlier_rate', 'owner_occupied_rate')
...
# Pivot demos longer
amount_long <- d_fema %>%
  pivot_longer(cols = amount_cols, names_to = 'variable', values_to = 'rates')
...
# Pivot demos longer
fema_long <- d_fema %>%
  pivot_longer(cols = fema_cols, names_to = 'variable', values_to = 'rates')
...
paste('Total tracts:', nrow(d_fema))

# Raw - valid water-levels
water_puerto <- d_puerto[waterlevel >0, .(count = .N), keyby = censustractid]
paste('Raw FEMA - tracts with valid waterlevel:', nrow(water_puerto))

# FEMA - missing water-levl
fema_water <- d_puerto[ , .(waterlevel = mean(waterlevel, rm.na = TRUE)), keyby = censustractid]
paste('Aggregated - tracts with missing waterlevel:', nrow(fema_water[is.na(waterlevel), ]))
```
