...
## Eligibility Status and Estiamted Damages
...
d_fema <- d_puerto[ , .(householdcomposition = as.numeric(mean(householdcomposition)),
                      ...
                      keyby = c('state', 'county', 'county_fips', 'censustractid')]
head(d_fema)
```
...
d_fema[ , `:=` (specialneeds_cap = specialneeds/total_population,
                ...
                personalpropertyeligible_cap = personalpropertyeligible/total_population),
              keyby = c('state', 'county', 'county_fips', 'censustractid')]

head(d_fema)
```

## Additional Data for Exploration
...
```{r}
# Read in CDC's Social Vulnerability Index (SVI) data
...
demo_cols <- c('below_poverty_rate', 'unemployed_labor_rate', 'bachelors_degree_rate',
