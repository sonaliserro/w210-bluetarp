...
```{r}
# Read csv
d <- fread(file = '../data/census-tract/census-tract-demographics.csv', encoding = 'UTF-8')

# Create FIPS code
d[ , `:=` (fips_code = substr(tractid, 1, 5))]

```

```{r}
# Aggregate by county
d_county <- d[ , .(total_population = sum(total_population),
                   ...
                   below_poverty_rate = sum(below_poverty_rate),
                   total_households_poverty = sum(total_households_poverty)), keyby = c('fips_code', 'county', 'state')]

# Write to csv
fwrite(d_county, file = '../data/census-tract/county-lookup-demographics.csv')
```
