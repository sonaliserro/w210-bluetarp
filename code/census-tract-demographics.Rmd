...
read_data <- function(file){
  ...
  #  d[d == '-'] <- NA       # median estimate - the median falls in the lowest interval of an open-ended distribution.
  #  d[d == '+'] <- NA       # median estimate - the median falls in the upper interval of an open-ended distribution.
...
```{r}
# Read in data
d_full <- fread(file = '../data/census-tract/census-tract-demographics.csv', encoding='UTF-8')
d_census <- d_full

# Reformat variable
d_census[ , `:=` (median_earnings_total = as.numeric(median_earnings_total))]
```


```{r}
# Keep key demographics
d_census <- d_census[ , c('tractid',
                          'below_poverty_rate',
...
```{r}
# Drop NAs
d_valid <- d_census[!is.na(below_poverty_rate), c('tractid', 'below_poverty_rate')]

# Calculate percentile rank
d_valid <- within(d_valid, 'poverty_rank' <- percent_rank_asc(d_valid$below_poverty_rate))
d_valid <- d_valid[ , c('tractid', 'poverty_rank')]

d_full <- merge(x = d_census, y = d_valid, by = 'tractid', all.x = TRUE)
```
...
```{r}
# Drop NAs
d_valid <- d_census[!is.na(unemployed_labor_rate), c('tractid', 'unemployed_labor_rate')]

# Calculate percentile rank
d_valid <- within(d_valid, 'unemployed_rank' <- percent_rank_asc(d_valid$unemployed_labor_rate))
d_valid <- d_valid[ , c('tractid', 'unemployed_rank')]

d_full <- merge(x = d_full, y = d_valid, by = 'tractid', all.x = TRUE)
```
...
```{r}
# Drop NAs
d_valid <- d_census[!is.na(median_earnings_total), c('tractid', 'median_earnings_total')]

# Calculate percentile rank
d_valid <- within(d_valid, 'earnings_rank' <- percent_rank_desc(d_valid$median_earnings_total))
d_valid <- d_valid[ , c('tractid', 'earnings_rank')]
...
```{r}
# Drop NAs
d_valid <- d_census[!is.na(owner_occupied_rate), c('tractid', 'owner_occupied_rate')]

# Calculate percentile rank
d_valid <- within(d_valid, 'owner_rank' <- percent_rank_asc(d_valid$owner_occupied_rate))
d_valid <- d_valid[ , c('tractid', 'owner_rank')]
...
```{r}
# Drop NAs
d_valid <- d_census[!is.na(built_1979_or_earlier_rate), c('tractid', 'built_1979_or_earlier_rate')]

# Calculate percentile rank
d_valid <- within(d_valid, 'house_old_rank' <- percent_rank_asc(d_valid$built_1979_or_earlier_rate))
d_valid <- d_valid[ , c('tractid', 'house_old_rank')]
...
d_full[ ,`:=` (dvi_raw = poverty_rank + unemployed_rank + earnings_rank + owner_rank + house_old_rank)]

# Keep ranks and index
d_trim <- d_full[ , c('tractid', 'poverty_rank', 'unemployed_rank', 'earnings_rank',
