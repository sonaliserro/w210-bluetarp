---
title: 'Census Tract Demographics - Florida, Texas, Puerto Rico, North Carolina'
...
## Omit Record with NAs

```{r}
# Write to csv
d_trim <- fread(file = '../data/census-tract/census-tract-demographics.csv', encoding = 'UTF-8')

d_trim <- na.omit(d_trim, cols=c('below_poverty_rate', 'unemployed_labor_rate', 'median_earnings_total', 'owner_occupied_rate', 'built_1979_or_earlier_rate'))

# Write to csv
fwrite(d_trim, file = '../data/census-tract/census-tract-demographics-trim.csv')
```
...
# Disaster Vulnerability Index (no income)
...
```{r}
# Raw Disaster Vulnerability Index (SVI)
d_rank[ ,`:=` (dvi_raw = poverty_rank + unemployed_rank + owner_rank + house_old_rank)]

# Keep ranks and index
d_trim <- d_rank[ , c('censusid', 'poverty_rank', 'unemployed_rank',
                      'owner_rank', 'house_old_rank', 'dvi_raw')]
...
```{r}
# Write to csv
fwrite(d_trim, file = '../data/census-tract/census-tract-dvi-v2.csv')
```
...
```{r}
# # Write to csv
# fwrite(d_full, file = '../data/census-tract/census-tract-demographics-v2.csv')
```
...
```{r}
# # Write to csv
# d_trim <- fread(file = '../data/census-tract/census-tract-demographics-v2.csv', encoding = 'UTF-8')
# 
# d_trim <- na.omit(d_trim, cols=c('below_poverty_rate', 'unemployed_labor_rate', 'median_earnings_total', 'owner_occupied_rate', 'built_1979_or_earlier_rate', 'dvi'))
# 
# # Write to csv
# fwrite(d_trim, file = '../data/census-tract/census-tract-demographics-trim-v2.csv')
```
