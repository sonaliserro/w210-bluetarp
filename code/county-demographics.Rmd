...
filenames <- list.files("../data/census/", pattern="*.csv", full.names=TRUE)
...
```{r}
## Fema counties
# d_fema <- fread(file = '../data/FEMA-DR-4339-PR.csv')
# county_base <- d_fema[ , .(count = .N), keyby = county]
# fwrite(county_base, file = '../data/county-fips-lookup.csv')

## Proper county names with fips
# d_pop <- fread(file = '../data/Puerto Rico Community Survey - Population 5year.csv', skip = 1, encoding = 'UTF-8')
...
## Write to csv
# fwrite(fips_lookup, file = '../data/fips_lookup.csv')
```

```{r}
## Above county files combined manually 
# county_lookup <- fread(file = '../data/county-fips-lookup.csv', encoding = 'UTF-8')
# county_lookup
```
